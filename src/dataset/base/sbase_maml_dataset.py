...
def load_file(filename: str) -> List:
    with open(filename, 'r') as reader:
        return [ujson.loads(line) for line in reader]
...
def load_data(dataset_files: Dict, mode: str) -> Dict:
    ...
    for key, filenames in dataset_files.items():
        for fl in filenames:
            LOGGER.info("mode:{} keys: {}/{} file: {}/{}  ".format(mode, key, dataset_files.keys(),
                                                                   filenames.index(fl) + 1, len(filenames)))
            data[key].extend(load_file(fl))
    return data


class sBaseMAMLDataset(Dataset):
    ...
    def multiprocessing(cls, **params: Dict, ) -> Any:
        instance = cls(
            ...
            support_size=params.get('support_size', False),  # support_size
            query_size=params.get('query_size', False),  # query_size
    ...
    def _construct(self, file_dir: str, data_lng: str, code_modalities: List[str], mode: str, token_dicts: TokenDicts,
        ...
        self.query_size = kwargs['query_size']
        ...
        if self.mode == 'train':
            if portion is None or portion == 1.0:
                ...
                random_indices = range(self.LENGTH)
                for key, value in self.data.items():
    ...
    def __getitem__(self, index: int) -> Dict:
        ...
        sample['support_size'] = self.support_size
        sample['query_size'] = self.query_size
        return sample


def _sbase_collate_fn(batch_data: List) -> Dict:
...
def sbase_maml_collate_fn(batch_data: List):
    support_size = batch_data[0]['support_size']
    query_size = batch_data[0]['query_size']
    assert len(batch_data) == support_size + query_size
    support_data = _sbase_collate_fn(batch_data[:support_size])
    query_data = _sbase_collate_fn(batch_data[support_size:])
