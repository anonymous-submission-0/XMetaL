...
def build_model(config: Dict, model: IModel, ) -> Module:
    ...
    def _load_pretrained_weights():
        if (config['common']['init_weights'] is None) or \
            (not os.path.exists(config['common']['init_weights'])) or \
            (config['common']['task'] == 'maml'):
            _init_param(model)
...
def load_config_dataset(args: namedtuple,
    ...
    params = {
        ...
        'src_xlang': src_xlang,
        # dtrl
        'src_other': 1.0 if config['dataset']['src_portion'] is None else config['dataset']['src_portion'],
        'src_ruby': 1.0 if config['dataset']['portion'] is None else config['dataset']['portion'],
        # maml
        'tgt_batch_size': config['training'].get('tgt_batch_size', batch_size),
        'support_size': config['training'].get('support_size', None),
        'query_size': config['training'].get('query_size', None),
    }

    if 'src_portion' in config['dataset'] and src_dataset_info is not None:
        src_dataset_info['portion'] = config['dataset']['src_portion']
    dataset = dataloader(
...
def save_checkpoints(optimizer: Optimizer, meta_optimizer: Optimizer, \
                     model: IModel, epoch: int, file_name: str):
    checkpoints = {
...
def load_checkpoints(file_name: str) -> Dict:
